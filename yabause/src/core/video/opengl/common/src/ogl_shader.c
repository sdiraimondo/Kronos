...
static const char fblitnear_img[] =
  ...
  static const char fbobsecure_img[] =
    ...
    "if (distance(val1, val2) > 0.3f) return mix(val1,val2,vec4(0.5));\n"
    "vec4 val3 = texelFetch( textureSampler, ivec2(coord.x+1,coord.y-1) , 0 ); \n"
    "vec4 val4 = texelFetch( textureSampler, ivec2(coord.x+1,coord.y+1) , 0 ); \n"
          "vec4 interpol = mix( \n"
              "mix( val1, val2,vec4(0.5)), \n"
              "mix( val3, val4,vec4(0.5)), \n"
              "vec4(0.5) \n"
            "); \n"
